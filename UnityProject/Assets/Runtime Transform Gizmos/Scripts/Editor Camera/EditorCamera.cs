    ...
    public class EditorCamera : MonoSingletonBase<EditorCamera>
    {
        ...
        private void MoveCameraBasedOnUserInput()
        {
            if(_mouse.IsRightMouseButtonDown)
            {
                ...
                if (Input.GetKey(KeyCode.W) && !_camera.orthographic) cameraTransform.position += cameraTransform.forward * moveAmount;
                else if (Input.GetKey(KeyCode.S) && !_camera.orthographic) cameraTransform.position -= cameraTransform.forward * moveAmount;
