...
namespace RTEditor
{
    ...
    public class TranslationGizmo : Gizmo
    {
        ...
        /// </summary>
        [SerializeField]
        private Color _selectedMultiAxisSquareColor = new Color(1.0f, 1.0f, 0.0f, 0.2f);

        /// <summary>
        /// This is the color of the selected multi-axis square lines.
        /// </summary>
        [SerializeField]
        private Color _selectedMultiAxisSquareLineColor = new Color(1.0f, 1.0f, 0.0f, 1.0f);
        ...
        /// <summary>
        /// When '_translateAlongCameraRightAndUpVectors' is true, this variable will be used to
        /// draw a square at the center of the gizmo which allows the user to translate along the
        /// camera's right and up vectors respectively. It represents the length of the square sides
        /// in screen units (both width and height).
        /// </summary>
        [SerializeField]
        private float _screenSizeOfCameraAxesTranslationSquare = 25.0f;

        /// <summary>
        /// This is the color that is used to draw the lines that make up the square used to translate
        /// along the camera right and up vectors. 
        /// </summary>
        [SerializeField]
        private Color _colorOfCameraAxesTranslationSquareLines = Color.white;

        /// <summary>
        /// Same as '_colorOfCameraAxesTranslationSquareLines', but it applies when the square is selected.
        /// </summary>
        [SerializeField]
        private Color _colorOfCameraAxesTranslationSquareLinesWhenSelected = Color.yellow;

        /// <summary>
        /// When vertex snapping is enabled, this variable will be used to draw a square at the center
        /// of the gizmo which allows the user to perform vertex snapping. It represents the length of
        /// the square sides in screen units (both width and height).
        /// </summary>
        [SerializeField]
        private float _screenSizeOfVertexSnappingSquare = 25.0f;

        /// <summary>
        /// This is the color that is used to draw the lines that make up the vertex snapping square.
        /// </summary>
        [SerializeField]
        private Color _colorOfVertexSnappingSquareLines = Color.white;

        /// <summary>
        /// Same as '_colorOfVertexSnappingSquareLines', but it applies when the square is selected.
        /// </summary>
        [SerializeField]
        ...
        /// <summary>
        /// Materials which are used for rendering the axis arrow cones. 2 arrays are used
        /// because 2 versions of the same material are needed: one is needed to render lit
        /// cones (when '_areArrowConesLit' is true) and the other is needed to render unlit 
        /// cones (when '_areArrowConesLit' is false).
        /// </summary>
        private Material[] _litArrowConeMaterials = new Material[3];
        private Material[] _unlitArrowConeMaterials = new Material[3];

        /// <summary>
        /// The materials which are needed to render the multi-axis squares.
        /// </summary>
        private Material[] _multiAxisSquareMaterials = new Material[3];

        /// <summary>
        /// This is the mesh which is used to render the arrow cones which sit at the tip of each translation axis.
        /// </summary>
        private Mesh _arrowConeMesh;

        /// <summary>
        /// This is the mesh which is used to render the multi-axis squares.
        /// </summary>
        ...
        public float ArrowConeRadius
        {
            ...
            set
            {
                // Store the new arrow cone radius, and if the application is playing, recreate the arrow cone mesh
                _arrowConeRadius = Mathf.Max(MinArrowConeRadius, value);
                if (Application.isPlaying) CreateArrowConeMesh();
            }
        }
        ...
        public float ArrowConeLength
        {
            ...
            set
            {
                // Store the new arrow cone length, and if the application is playing, recreate the arrow cone mesh
                _arrowConeLength = Mathf.Max(MinArrowConeLength, value);
                if (Application.isPlaying) CreateArrowConeMesh();
            }
        }
        ...
        public float MultiAxisSquareSize
        {
            ...
            set
            {
                // Store the new size, and if the application is playing, recreate the multi-axis square mesh
                _multiAxisSquareSize = Mathf.Max(MinMultiAxisSquareSize, value);
                if (Application.isPlaying) CreateMultiAxisSquareMesh();
            }
        }
        ...
        /// <summary>
        /// Gets/sets the screen size (width and height) of the square which can be used to translate along the camera
        /// right and up axes. The minimum value for the square size is given by the 'MinScreenSizeOfCameraAxesTranslationSquare'.
        /// Values smaller than that will be clamped accordingly.
        /// </summary>
        public float ScreenSizeOfCameraAxesTranslationSquare { get { return _screenSizeOfCameraAxesTranslationSquare; } set { _screenSizeOfCameraAxesTranslationSquare = Mathf.Max(MinScreenSizeOfCameraAxesTranslationSquare, value); } }
        
        /// <summary>
        /// Gets/sets the color of the camera axes translation square.
        /// </summary>
        public Color ColorOfCameraAxesTranslationSquareLines { get { return _colorOfCameraAxesTranslationSquareLines; } set { _colorOfCameraAxesTranslationSquareLines = value; } }

        /// <summary>
        /// Gets/sets the color of the camera axes translation square when the square is selected.
        /// </summary>
        public Color ColorOfCameraAxesTranslationSquareLinesWhenSelected { get { return _colorOfCameraAxesTranslationSquareLinesWhenSelected; } set { _colorOfCameraAxesTranslationSquareLinesWhenSelected = value; } }
       
        /// <summary>
        /// Gets/sets the size (width and height) of the vertex snapping square. The minimum value for the square size is
        /// given by the 'MinScreenSizeOfVertexSnappingSquare' property. Values smaller than that will be clamped accordingly.
        /// </summary>
        public float ScreenSizeOfVertexSnappingSquare { get { return _screenSizeOfVertexSnappingSquare; } set { _screenSizeOfVertexSnappingSquare = Mathf.Max(value, MinScreenSizeOfVertexSnappingSquare); } }
        
        /// <summary>
        /// Gets/sets the color of the vertex snapping square lines.
        /// </summary>
        public Color ColorOfVertexSnappingSquareLines { get { return _colorOfVertexSnappingSquareLines; } set { _colorOfVertexSnappingSquareLines = value; } }

        /// <summary>
        /// Gets/sets the color of the vertex snapping square lines when the square is selected.
        /// </summary>
        public Color ColorOfVertexSnappingSquareLinesWhenSelected { get { return _colorOfVertexSnappingSquareLinesWhenSelected; } set { _colorOfVertexSnappingSquareLinesWhenSelected = value; } }
        ...
        public override bool IsReadyForObjectManipulation()
        {
            ...
            //  e) if vertex snapping is enabled. (_snapSettings.IsVertexSnappingEnabled)
            return _selectedAxis != GizmoAxis.None || _selectedMultiAxisSquare != MultiAxisSquare.None || 
                   _isCameraAxesTranslationSquareSelected || _snapSettings.IsVertexSnappingEnabled;
        }
        ...
        protected override void Start()
        {
            ...
            CreateArrowConeMesh();              // Create the mesh which is used to render the arrow cones that sit at the tip of each axis
            CreateMultiAxisSquareMesh();        // Create the mesh which is used to render the multi-axis squares

            CreateArrowConeMaterials();         // Create the materials which are needed to render the arrow cones
            CreateMultiAxisSquareMaterials();   // Create the materials which are needed to render the multi-axis squares
        }
        ...
        protected override void Update()
        {
            ...
            if (!_translateAlongCameraRightAndUpAxes && !_snapSettings.IsVertexSnappingEnabled)
            {
                Matrix4x4[] multiAxisWorldTransforms = GetMultiAxisSquaresWorldTransforms();
                DrawMultiAxisSquares(multiAxisWorldTransforms);
            }

            // When vertex snapping is not enabled, we will update the selection status of the gizmo components
            if(!_snapSettings.IsVertexSnappingEnabled)
            {
                ...
                for (int axisIndex = 0; axisIndex < 3; ++axisIndex)
                {
                    ...
                    if (!axisWasPicked && pickRay.IntersectsCone(_arrowConeRadius, _arrowConeLength, arrowConeWorldTransforms[axisIndex], out t))
                    {
                ...
                // Check if the camera axes translation square is selected. We only do this if the square is active.
                if (_translateAlongCameraRightAndUpAxes && IsMouseCursorInsideCameraAxesTranslationSquare())
                {
            ...
            // Make sure the arrow cone materials have their colors updated accordingly
            for (int axisIndex = 0; axisIndex < 3; ++axisIndex)
            {
                Material arrowConeMaterial = GetArrowConeMaterial((GizmoAxis)axisIndex);
                arrowConeMaterial.SetColor("_Color", _selectedAxis == (GizmoAxis)axisIndex ? _selectedAxisColor : _axesColors[axisIndex]);
            }

            // Make sure the multi-axis materials have their colors updated accordingly
            for (int multiAxisIndex = 0; multiAxisIndex < 3; ++multiAxisIndex)
            {
                Material multiAxisMaterial = _multiAxisSquareMaterials[multiAxisIndex];
                multiAxisMaterial.SetColor("_Color", _selectedMultiAxisSquare == (MultiAxisSquare)multiAxisIndex ? _selectedMultiAxisSquareColor : _multiAxisSquareColors[multiAxisIndex]);
            }
        }
        ...
        private Vector3 GetWorldPositionClosestToMouseCursorForVertexSnapping(List<GameObject> gameObjects, bool considerOnlyMeshObjects)
        {
            ...
            if(objectsWithMesh.Count != 0)
            {
                ...
                var gameObjectToVertexGroup = new Dictionary<GameObject, List<MeshVertexGroup>>();
                foreach(GameObject gameObject in gameObjects)
                {
                    // If the object's mesh contains a mapping, add an entry in the dictionary
                    Mesh objectMesh = gameObject.GetMesh();
                    if (meshVertexGroupMappings.ContainsMappingForMesh(objectMesh))
                    {
                        gameObjectToVertexGroup.Add(gameObject, meshVertexGroupMappings.GetMeshVertexGroups(objectMesh));
                    }
                }
                ...
                foreach(var pair in gameObjectToVertexGroup)
                {
        ...
        protected override void OnMouseMoved()
        {
            ...
            if (_mouse.IsLeftMouseButtonDown)
            {
                ...
                if(_snapSettings.IsVertexSnappingEnabled)
                {
                    ...
                    // Retrieve the game objects which can be used for snapping
                    List<GameObject> objectsClosestToCursor = GetObjectsForClosestVertexSelection(visibleGameObjects);
        ...
        protected override void OnRenderObject()
        {
            ...
            foreach (int axisIndex in axisIndices)
            {
                ...
                // Draw the axis line
                GLPrimitives.Draw3DLine(startPoint, endPoint, axisColor, _lineRenderingMaterial);
            }

            // Now draw the multi-axis square lines that surround the multi-axis squares.
            // Note: We will not draw the lines if the camera axes translation square is enabled or if vertex snapping is active.
            if (!_translateAlongCameraRightAndUpAxes && !_snapSettings.IsVertexSnappingEnabled)
            {
                ...
                GLPrimitives.Draw3DLines(squareLinesPoints, squareLinesColors, false, _lineRenderingMaterial, false, Color.black);
            }

            // Draw the camera axes translation square.
            // Note: We only draw if the user has activated the square.
            if (_translateAlongCameraRightAndUpAxes)
            {
                // Establish the color which must be used to draw the square lines
                Color squareLineColor = _isCameraAxesTranslationSquareSelected ? _colorOfCameraAxesTranslationSquareLinesWhenSelected : _colorOfCameraAxesTranslationSquareLines;

                // Draw
                GLPrimitives.Draw2DRectangleBorderLines(GetCameraAxesTranslationSquareScreenPoints(), squareLineColor, _lineRenderingMaterial, _camera);
            }

            // Draw the vertex snapping square if necessary
            if(_snapSettings.IsVertexSnappingEnabled)
            {
                // Establish the color used to draw the square lines
                Color squareLineColor = _mouse.IsLeftMouseButtonDown ? _colorOfVertexSnappingSquareLinesWhenSelected : _colorOfVertexSnappingSquareLines;

                // Draw
                GLPrimitives.Draw2DRectangleBorderLines(GetVertexSnappingSquareScreenPoints(), squareLineColor, _lineRenderingMaterial, _camera);
            }
        }
        #endregion
        ...
        /// <summary>
        /// Returns the screen space points which form the camera axes translation square.
        /// </summary>
        private Vector2[] GetCameraAxesTranslationSquareScreenPoints()
        {
            ...
            float halfSquareSize = _screenSizeOfCameraAxesTranslationSquare * 0.5f;
        ...
        /// <summary>
        /// Returns the screen space points which form vertex snapping square.
        /// </summary>
        private Vector2[] GetVertexSnappingSquareScreenPoints()
        {
            ...
            // center in screen space by half the square size along the screen right and up axes.
            Vector2 screenSpaceSquareCenter = _camera.WorldToScreenPoint(_gizmoTransform.position);
            float halfSquareSize = _screenSizeOfVertexSnappingSquare * 0.5f;
        ...
        /// of the camera axes translation square.
        /// </summary>
        ...
        private void DrawMultiAxisSquares(Matrix4x4[] worldTransformMatrices)
        {
            ...
            for (int multiAxisIndex = 0; multiAxisIndex < 3; ++multiAxisIndex)
            {
                Graphics.DrawMesh(_multiAxisSquareMesh, worldTransformMatrices[multiAxisIndex], _multiAxisSquareMaterials[multiAxisIndex], gameObject.layer);
            }
        }
        ...
        private Matrix4x4[] GetMultiAxisSquaresWorldTransforms()
        {
            ...
            for (int multiAxisIndex = 0; multiAxisIndex < 3; ++multiAxisIndex)
            {
                ...
                worldTransforms[multiAxisIndex].SetTRS(worldPosition, worldRotation, _gizmoTransform.lossyScale);
            }
        ...
        /// <summary>
        /// Creates the materials which are used to render the arrow cone objects.
        /// </summary>
        private void CreateArrowConeMaterials()
        {
            // Create the material shader instances
            Shader litConeShader = Shader.Find("Gizmo Lit Mesh");
            Shader unlitConeShader = Shader.Find("Gizmo Unlit Mesh");

            // Create a material for each axis.
            // Note: We create 2 materials for each axis. One material will be used when the arrow cones
            //       must be lit and the other one will be used otherwise.
            for (int axisIndex = 0; axisIndex < 3; ++axisIndex)
            {
                _litArrowConeMaterials[axisIndex] = new Material(litConeShader);
                _litArrowConeMaterials[axisIndex].SetInt("_StencilRefValue", _axesStencilRefValues[axisIndex]);

                _unlitArrowConeMaterials[axisIndex] = new Material(unlitConeShader);
                _unlitArrowConeMaterials[axisIndex].SetInt("_StencilRefValue", _axesStencilRefValues[axisIndex]);
            }
        }

        /// <summary>
        /// Retrieves the material which must be used to render the arrow cone that corresponds
        /// to the specified gizmo axis.
        /// </summary>
        private Material GetArrowConeMaterial(GizmoAxis gizmoAxis)
        {
            if (_areArrowConesLit) return _litArrowConeMaterials[(int)gizmoAxis];
            else return _unlitArrowConeMaterials[(int)gizmoAxis];
        }

        /// <summary>
        /// Creates the materials which are used to render the multi-axis squares.
        /// </summary>
        private void CreateMultiAxisSquareMaterials()
        {
            // Create the material shader and then use it to create the multi-axis materials
            Shader multiAxisShader = Shader.Find("Gizmo Unlit Mesh");
            for (int multiAxisIndex = 0; multiAxisIndex < 3; ++multiAxisIndex)
            {
                _multiAxisSquareMaterials[multiAxisIndex] = new Material(multiAxisShader);
            }
        }

        /// <summary>
        /// Creates the mesh which is used to render the multi-axis squares.
        /// </summary>
        private void CreateMultiAxisSquareMesh()
        {
            // Destroy the mesh if it already exists and then create it.
            // Note: It is important to destroy the old mesh because this method might be called in
            //       response to a user's request to change the square's properties (e.g. size) and 
            //       in that case the old mesh has to be destroyed and a new one created.
            if (_multiAxisSquareMesh != null) DestroyGizmoMesh(_multiAxisSquareMesh);
            _multiAxisSquareMesh = ProceduralMeshGenerator.CreatePlaneMesh(_multiAxisSquareSize, _multiAxisSquareSize);
        }

        /// <summary>
        /// Creates the cone mesh that is used to render the arrow cone objects.
        /// </summary>
        private void CreateArrowConeMesh()
        {
            // You can change these if needed
            const int numberOfVerticalSlices = 20;
            const int numberOfHorizontalSlices = 20;
            const int bottomCapRings = 5;

            // Destroy the mesh if it already exists and then create it.
            // Note: It is important to destroy the old mesh because this method might be called in
            //       response to a user's request to change the cone's properties (e.g. radius) and 
            //       in that case the old mesh has to be destroyed and a new one created.
            if (_arrowConeMesh != null) DestroyGizmoMesh(_arrowConeMesh);
            _arrowConeMesh = ProceduralMeshGenerator.CreateConeMesh(_arrowConeRadius, _arrowConeLength, numberOfVerticalSlices, numberOfHorizontalSlices, bottomCapRings);
        }
        ...
        private void DrawArrowCones(Matrix4x4[] worldTransformMatrices)
        {
            ...
            for (int axisIndex = 0; axisIndex < 3; ++axisIndex)
            {
                Graphics.DrawMesh(_arrowConeMesh, worldTransformMatrices[axisIndex], GetArrowConeMaterial((GizmoAxis)axisIndex), gameObject.layer);
            }
        }
        ...
        private Matrix4x4[] GetArrowConesWorldTransforms()
        {
            ...
            for (int axisIndex = 0; axisIndex < 3; ++axisIndex)
            {
                ...
                worldTransforms[axisIndex].SetTRS(worldPosition, worldRotation, _gizmoTransform.lossyScale);
            }
