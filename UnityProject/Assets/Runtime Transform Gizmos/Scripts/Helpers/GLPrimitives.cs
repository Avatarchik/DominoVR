...
namespace RTEditor
{
    ...
    public static class GLPrimitives
    {
        ...
        /// <summary>
        /// Draws wire selection boxes given the specified selection box collection. When drawing a wire box, the function 
        /// will draw a line between all adjacent box corner points.
        /// </summary>
        /// <param name="selectionBoxes">
        /// The selection boxes which must be drawn.
        /// </param>
        /// <param name="boxSizeScaleFactor">
        /// This is a scale factor which must be applied to a box's size before rendering. This scale is applied
        /// in addition to the scale information which is already present inside the boxes' transform matrices.
        /// </param>
        /// <param name="camera">
        /// The camera which is used to render the box lines.
        /// </param>
        /// <param name="lineColor">
        /// The color which must be used to render the box wire lines.
        /// </param>
        /// <param name="boxLineMaterial">
        /// The material which must be used to render the box lines.
        /// </param>
        public static void DrawWireSelectionBoxes(List<ObjectSelectionBox> selectionBoxes, float boxSizeScaleFactor, Camera camera, Color lineColor, Material boxLineMaterial)
        {
            ...
            for (int selectionBoxIndex = 0; selectionBoxIndex < selectionBoxes.Count; ++selectionBoxIndex)
            {
                ...
                Bounds modelSpaceBox = objectSelectionBox.ModelSpaceBox;
                Matrix4x4 boxTransformMatrix = objectSelectionBox.TransformMatrix;

                // Apply the box scale factor
                modelSpaceBox.size *= boxSizeScaleFactor;
        ...
        /// <summary>
        /// Given a collection of object selection boxes, the function will draw lines at the corners of each box that 
        /// connect with adjacent corner points. 
        /// </summary>
        /// <param name="selectionBoxes">
        /// The selection boxes which must be drawn.
        /// </param>
        ...
        /// in addition to the scale information which is already present inside the boxes' transform matrices.
        /// </param>
        /// <param name="cornerLineLength">
        /// The length of the corner lines. If the line length exceeds half the box size on any axis, the function
        /// will clamp the length such that there will always be a little bit of space left between 2 lines which
        /// exist on the same edge of the box.
        /// </param>
        /// <param name="camera">
        /// The camera which is used to render the box lines.
        /// </param>
        /// <param name="cornerLineColor">
        /// The color which must be used to render the box corner lines.
        /// </param>
        /// <param name="boxLineMaterial">
        /// The material which must be used to render the box lines.
        /// </param>
        public static void DrawCornerLinesForSelectionBoxes(List<ObjectSelectionBox> selectionBoxes, float boxSizeScaleFactor, float cornerLineLength, Camera camera, Color cornerLineColor, Material boxLineMaterial)
        {
            // Save the current modelview matrix. We will always modify the model view matrix such that it 
            // represents the result of the multiplication between the box's transform matrix and the camera
            // view matrix.
            ...
            for (int selectionBoxIndex = 0; selectionBoxIndex < selectionBoxes.Count; ++selectionBoxIndex)
            {
                ...
                // Note: We will now scale the box size and center by the scale which is stored inside the box's transform matrix. This may appear 
                //       strange because after all, we will use the box transform to construct the modelview matrix that will be used for rendering. 
                //       But this is exactly the problem. If we take the scale information into consideration when constructing the modelview matrix, 
                //       the corner lines will get bigger or shorter (depending on the scale) and will loose their original length (the one which was
                //       specified as parameter to this function). We want to satisfy (as much as possible) the request to draw the corner lines at 
                //       the specified length (given by 'cornerLineLength'). So what we will do instead is to scale the box size and center by the 
                //       scale stored in the transform matrix and then eliminate the scale information from the transform matrix. This will make sure 
                //       that the box size is correct while also preserivng the corner line length.
                Vector3 matrixScale = boxTransformMatrix.GetScaleTransform();
                ...
                // Note: We will also scale the extents vector down a little bit so that we always get a litle
                //       bit of space between the lines which meet along the same axis.
                float lineLengthAlongX = Mathf.Min(cornerLineLength, modelSpaceBox.extents.x * 0.8f);
                float lineLengthAlongY = Mathf.Min(cornerLineLength, modelSpaceBox.extents.y * 0.8f);
                float lineLengthAlongZ = Mathf.Min(cornerLineLength, modelSpaceBox.extents.z * 0.8f);
