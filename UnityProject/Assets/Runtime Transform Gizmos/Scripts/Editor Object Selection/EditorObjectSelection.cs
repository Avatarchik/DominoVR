...
namespace RTEditor
{
    ...
    public class EditorObjectSelection : MonoSingletonBase<EditorObjectSelection>, IMessageListener
    {
        ...
        /// use this to render the object selection boxes.
        /// </summary>
        ...
        /// <summary>
        /// Returns the material which can be used to perform any necessary line rendering.
        /// </summary>
        public Material LineRenderingMaterial { get { return _lineRenderingMaterial; } }
        #endregion
        ...
        /// </summary>
        public Bounds GetWorldAABB()
        {
            // If no objects are selected, return an invalid AABB
            if (_selectedObjects.Count == 0) return BoundsExtensions.GetInvalidBoundsInstance();

            // Loop through all selected game objects
            Bounds selectionWorldAABB = BoundsExtensions.GetInvalidBoundsInstance();
            foreach (GameObject selectedObject in _selectedObjects)
            {
                ...
                Bounds objectWorldAABB = selectedObject.GetWorldSpaceAABB();
                if (objectWorldAABB.IsValid())
                {
                    ...
                    if (selectionWorldAABB.IsValid()) selectionWorldAABB.Encapsulate(objectWorldAABB);
                    else selectionWorldAABB = objectWorldAABB;
                }
            }

            // Return the AABB
            return selectionWorldAABB;
        }
        ...
        private GameObject GetPickedObject()
        {
            ...
            // Cast ray and return result
            RaycastHit rayHit;
            if (Physics.Raycast(camera.ScreenPointToRay(Input.mousePosition), out rayHit)) return rayHit.collider.gameObject;
            else return null;
        }
        ...
        private Vector3 CalculateSelectionWorldCenter()
        {
            ...
            else
            {
                ...
                foreach (GameObject selectedObject in _selectedObjects)
                {
                    ...
                    Bounds worldAABB = selectedObject.GetWorldSpaceAABB();
