...
namespace RTEditor
{
    ...
    public class ScaleGizmo : Gizmo
    {
        ...
        /// <summary>
        /// This is the mesh that is used to render the scale boxes.
        /// </summary>
        private Mesh _scaleBoxMesh;

        /// <summary>
        /// These 2 arrays hold the materials which will be used to render the scale boxes. 2 arrays
        /// are used because the user can choose whether or not the scale boxes are lit.
        /// </summary>
        private Material[] _litScaleBoxMaterials;
        private Material[] _unlitScaleBoxMaterials;

        /// <summary>
        /// This is the mesh that is used to render the multi-axis triangles.
        /// </summary>
        private Mesh _multiAxisTriangleMesh;

        /// <summary>
        /// The materials which are needed to render the multi-axis triangles.
        /// </summary>
        ...
        /// </summary>
        [SerializeField]
        private Color _selectedMultiAxisTriangleColor = new Color(1.0f, 1.0f, 0.0f, 0.2f);
        ...
        /// </summary>
        [SerializeField]
        private Color _selectedMultiAxisTriangleLineColor = new Color(1.0f, 1.0f, 0.0f, 1.0f);
        ...
        public float ScaleBoxWidth
        {
            ...
            set
            {
                // Store the width and, if the application is playing, make sure the scale box mesh is recreated
                _scaleBoxWidth = Mathf.Max(value, MinScaleBoxSize);
                if (Application.isPlaying) CreateScaleBoxMesh();
            }
        }
        ...
        public float ScaleBoxHeight
        {
            ...
            set
            {
                // Store the height and, if the application is playing, make sure the scale box mesh is recreated
                _scaleBoxHeight = Mathf.Max(value, MinScaleBoxSize);
                if (Application.isPlaying) CreateScaleBoxMesh();
            }
        }
        ...
        public float ScaleBoxDepth
        {
            ...
            set
            {
                // Store the depth and, if the application is playing, make sure the scale box mesh is recreated
                _scaleBoxDepth = Mathf.Max(value, MinScaleBoxSize);
                if (Application.isPlaying) CreateScaleBoxMesh();
            }
        }
        ...
        public float MultiAxisTriangleSideLength
        {
            ...
            set
            {
                // Store the new multi-axis triangle length and, if the application is playing, recreate the multi-axis triangle mesh
                _multiAxisTriangleSideLength = Mathf.Max(MinMultiAxisTriangleSideLength, value);
                if (Application.isPlaying) CreateMultiAxisTriangleMesh();
            }
        }
        ...
        protected override void Start()
        {
            ...
            CreateScaleBoxMaterials();              // Create the materials which are used to render the scale boxes
            CreateScaleBoxMesh();                   // Create the scale box mesh which is used to render the scale boxes

            CreateMultiAxisTriangleMaterials();     // Create the materials which are used to render the multi-axis triangles
            CreateMultiAxisTriangleMesh();          // Create the triangle mesh which is used to render the multi-axis triangles
        }
        ...
        protected override void Update()
        {
            ...
            // Note: The scale boxes are not actual game objects which exist in the scene. If that were the case,
            //       we wouldn't need to perform any drawing here. The same is available for the other gizmo
            //       components that we need to draw inside this method.
            ...
            for (int axisIndex = 0; axisIndex < 3; ++axisIndex)
            {
                ...
                if (!axisWasPicked && pickRay.IntersectsBox(_scaleBoxWidth, _scaleBoxHeight, _scaleBoxDepth, scaleBoxWorldTransforms[axisIndex], out t))
                {
                    // Calculate the intersection point and check if it is closer to the camera than what we have so far
                    Vector3 intersectionPoint = pickRay.origin + pickRay.direction * t;
                    float distanceFromCamera = (intersectionPoint - cameraPosition).magnitude;
            ...
            // Make sure the scale box materials have their colors updated to account for the new selection state
            for (int axisIndex = 0; axisIndex < 3; ++axisIndex)
            {
                Material scaleBoxMaterial = GetScaleBoxMaterial(axisIndex);
                scaleBoxMaterial.SetColor("_Color", _selectedAxis == (GizmoAxis)axisIndex ? _selectedAxisColor : _axesColors[axisIndex]);
            }

            // Make sure the multi-axis triangles materials have their colors updated to account for the new selection state
            for (int multiAxisIndex = 0; multiAxisIndex < 3; ++multiAxisIndex)
            {
                Material multiAxisMaterial = _multiAxisTriangleMaterials[multiAxisIndex];
                multiAxisMaterial.SetColor("_Color", _selectedMultiAxisTriangle == (MultiAxisTriangle)multiAxisIndex ? _selectedMultiAxisTriangleColor : _multiAxisTriangleColors[multiAxisIndex]);
            }
        }
        ...
        /// <summary>
        /// Creates all the necessary scale box materials.
        /// </summary>
        private void CreateScaleBoxMaterials()
        {
            // Create the shaders which will be associated with the materials
            Shader litMeshShader = Shader.Find("Gizmo Lit Mesh");
            Shader unlitMeshShdaer = Shader.Find("Gizmo Unlit Mesh");

            // Create the material arrays
            _litScaleBoxMaterials = new Material[3];
            _unlitScaleBoxMaterials = new Material[3];

            // Loop through each element inside both arrays and create the materials for each axis
            for (int axisIndex = 0; axisIndex < 3; ++axisIndex)
            {
                _litScaleBoxMaterials[axisIndex] = new Material(litMeshShader);
                _litScaleBoxMaterials[axisIndex].SetInt("_StencilRefValue", _axesStencilRefValues[axisIndex]);

                _unlitScaleBoxMaterials[axisIndex] = new Material(unlitMeshShdaer);
                _unlitScaleBoxMaterials[axisIndex].SetInt("_StencilRefValue", _axesStencilRefValues[axisIndex]);
            }
        }

        /// <summary>
        /// Creates the materials which are used to render the multi-axis triangles.
        /// </summary>
        private void CreateMultiAxisTriangleMaterials()
        {
            // Create the shader which must be associated with the materials
            Shader multiAxisShader = Shader.Find("Gizmo Unlit Mesh");

            // Create the multi-axis materials
            for (int multiAxisIndex = 0; multiAxisIndex < 3; ++multiAxisIndex)
            {
                _multiAxisTriangleMaterials[multiAxisIndex] = new Material(multiAxisShader);
            }
        }

        /// <summary>
        /// Creates the mesh that is used to draw the scale boxes which sit at the
        /// tip of each axis.
        /// </summary>
        private void CreateScaleBoxMesh()
        {
            ...
            //       response to a user's request to change the scale boxes' properties (e.g. width)
            //       and in that case the old mesh has to be destroyed and a new one created.
            if (_scaleBoxMesh != null) DestroyGizmoMesh(_scaleBoxMesh);
            _scaleBoxMesh = ProceduralMeshGenerator.CreateBoxMesh(_scaleBoxWidth, _scaleBoxHeight, _scaleBoxDepth);
        }

        /// <summary>
        /// Creates the mesh that is used to draw the multi-axist triangles that can
        /// be used to scale along 2 axes at once.
        /// </summary>
        private void CreateMultiAxisTriangleMesh()
        {
            // Destroy the mesh if it already exists and then create it.
            // Note: It is important to destroy the old mesh because this method might be called in
            //       response to a user's request to change the triangle's properties (e.g. side length)
            //       and in that case the old mesh has to be destroyed and a new one created.
            if (_multiAxisTriangleMesh != null) DestroyGizmoMesh(_multiAxisTriangleMesh);
            _multiAxisTriangleMesh = ProceduralMeshGenerator.CreateRightAngledTriangleMesh(_multiAxisTriangleSideLength, _multiAxisTriangleSideLength);
        }
        ...
        private void DrawAxesLines()
        {
            ...
            foreach (int axisIndex in axisIndices)
            {
                ...
                // Draw the axis line
                GLPrimitives.Draw3DLine(startPoint, endPoint, _selectedAxis == (GizmoAxis)axisIndex ? _selectedAxisColor : _axesColors[axisIndex], _lineRenderingMaterial);
            }
        }
        ...
        private void DrawScaleBoxes(Matrix4x4[] worldTransforms)
        {
            ...
            for (int axisIndex = 0; axisIndex < 3; ++axisIndex)
            {
                Graphics.DrawMesh(_scaleBoxMesh, worldTransforms[axisIndex], GetScaleBoxMaterial(axisIndex), gameObject.layer);
            }
        }

        /// <summary>
        /// Returns the material which must be used to render the scale box for the specified axis.
        /// </summary>
        private Material GetScaleBoxMaterial(int axisIndex)
        {
            // Return the material based on whether or not the scale boxes must be lit
            return _areScaleBoxesLit ? _litScaleBoxMaterials[axisIndex] : _unlitScaleBoxMaterials[axisIndex];
        }
        ...
        private void DrawMultiAxisTriangles(Matrix4x4[] worldTransforms)
        {
            ...
            for (int multiAxisIndex = 0; multiAxisIndex < 3; ++multiAxisIndex)
            {
                Graphics.DrawMesh(_multiAxisTriangleMesh, worldTransforms[multiAxisIndex], _multiAxisTriangleMaterials[multiAxisIndex], gameObject.layer);
            }
        }
        ...
        private Matrix4x4[] GetMultiAxisTrianglesWorldTransforms()
        {
            ...
            for (int multiAxisIndex = 0; multiAxisIndex < 3; ++multiAxisIndex)
            {
                ...
                Vector3 worldScale = Vector3.Scale(_gizmoTransform.lossyScale, localScales[multiAxisIndex]) * multiAxisTriangleScaleFactor;
        ...
        private void DrawMultiAxisTrianglesLines()
        {
            ...
            if (!_scaleAlongAllAxes)
            {
                ...
                // Draw the lines
                GLPrimitives.Draw3DLines(triangleLinesPoints, triangleLinesColors, false, _lineRenderingMaterial, false, Color.black);
            }
        }
        ...
        private void DrawAllAxesSquareLines()
        {
            ...
            if (_scaleAlongAllAxes)
            {
                ...
                // Draw
                GLPrimitives.Draw2DRectangleBorderLines(GetAllAxesScaleSquareScreenPoints(), squareLineColor, _lineRenderingMaterial, _camera);
            }
        }
        ...
        private void DrawObjectsLocalAxesDuringScaleOperation()
        {
            ...
            if (_mouse.IsLeftMouseButtonDown && _drawObjectsLocalAxesWhileScaling && IsReadyForObjectManipulation() && ControlledObjects != null)
            {
                ...
                GLPrimitives.Draw3DLines(axesLinesPoints, axesLinesColors, false, _lineRenderingMaterial, false, Color.black);
            }
        }
