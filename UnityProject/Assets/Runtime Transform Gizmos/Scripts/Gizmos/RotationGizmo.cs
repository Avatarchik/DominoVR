...
namespace RTEditor
{
    ...
    public class RotationGizmo : Gizmo
    {
        ...
        /// <summary>
        /// The material which must be used to render the rotation sphere when the user has
        /// requested that the sphere must be lit.
        /// </summary>
        private Material _litRotationSphereMaterial;

        /// <summary>
        /// The material which must be used to render the rotation sphere when the user has
        /// requested that the sphere must not be lit.
        /// </summary>
        private Material _unlitRotationSphereMaterial;

        /// <summary>
        /// This is the material which is used to render the rotation guide disc.
        /// </summary>
        private Material _rotationGuideDiscMaterial;

        /// <summary>
        /// This is the mesh which is used to render the rotation sphere.
        /// </summary>
        ...
        public float RotationSphereRadius
        {
            ...
            set
            {
                // Store the new radius and if the application is currently playing, make sure that the internal data is updated accordingly. 
                // This includes the rotatino sphere mesh and the gizmo local space vertices which make up the rotation circles.
                _rotationSphereRadius = Mathf.Max(MinRotationSphereRadius, value);
                if (Application.isPlaying)
                {
                    CalculateRotationCirclePointsInGizmoLocalSpace();
                    CreateRotationSphereMesh();
                }
            }
        }
        ...
        public Color RotationSphereColor
        {
            ...
            set
            {
                // Store the new color and if the application is currently playing, make sure
                // that the sphere materials are updated accordingly.
                _rotationSphereColor = value;
                if (Application.isPlaying) AssignRotationSphereColorToSphereMaterials();
            }
        }
        ...
        public Color RotationGuideDiscColor
        {
            ...
            set
            {
                // Store the rotation disc color and if the application is playing, assign the color
                // to the material which is used to render the rotation disc.
                _rotationGuideDiscColor = value;
                if (Application.isPlaying) _rotationGuideDiscMaterial.SetColor("_Color", _rotationGuideDiscColor);
            }
        }
        ...
        protected override void Start()
        {
            ...
            CreateRotationSphereMaterials();                    // Create the materials which are needed to render the rotation sphere (lit and unlit)
            AssignRotationSphereColorToSphereMaterials();       // Make sure the rotation sphere color is assigned to the material which is used to render the rotation sphere

            CreateRotationSphereMesh();                         // Create the rotation sphere mesh 

            CreateRotationGuideDiscMaterial();                  // Create the material which is used to render the rotation guide disc
        ...
        protected override void Update()
        {
            ...
            // Submit the rotation sphere mesh for drawing
            // Note: The rotation sphere is not an actual game object which exists in the scene. If that were the case,
            //       we wouldn't need to perform any drawing here.
        ...
        protected override void OnRenderObject()
        {
            ...
            if (_mouse.IsLeftMouseButtonDown)
            {
                ...
                if (_selectedAxis != GizmoAxis.None && _showRotationGuide)
                {
                    ...
                    GLPrimitives.Draw3DLine(_gizmoTransform.position, _rotationGuideLinePoints[0], _rotationGuieLineColor, _lineRenderingMaterial);
                    GLPrimitives.Draw3DLine(_gizmoTransform.position, _rotationGuideLinePoints[1], _rotationGuieLineColor, _lineRenderingMaterial);
                    ...
                    GLPrimitives.Draw3DFilledDisc(_gizmoTransform.position, _rotationGuideLinePoints[0], _rotationGuideLinePoints[1], discPlaneNormal, _rotationGuideDiscColor, _rotationGuideDiscMaterial);
                }
                else
                if ((_isCameraLookRotationCircleSelected && _showRotationGuide))
                {
                    ...
                    GLPrimitives.Draw2DLine(rotationSphereCenterInScreenSpace, _rotationGuideLinePoints[0], _rotationGuieLineColor, _lineRenderingMaterial, _camera);
                    GLPrimitives.Draw2DLine(rotationSphereCenterInScreenSpace, _rotationGuideLinePoints[1], _rotationGuieLineColor, _lineRenderingMaterial, _camera);

                    // Draw the guide disc. The disc will also be rendered in screen space.
                    GLPrimitives.Draw2DFilledDisc(rotationSphereCenterInScreenSpace, _rotationGuideLinePoints[0], _rotationGuideLinePoints[1], _rotationGuideDiscColor, _rotationGuideDiscMaterial, _camera);
                }
            }
            ...
            if ((_showRotationSphereBoundary || _showCameraLookRotationCircle) && IsGizmoVisible())
            {
                ...
                if (_showRotationSphereBoundary) GLPrimitives.Draw2DCircleBorderLines(rotationSphereScreenSpaceBoundaryPoints, sphereCenterInScreenSpace, _rotationSphereBoundaryLineColor, 1.0f, _lineRenderingMaterial, _camera);
                if (_showCameraLookRotationCircle)
                {
                    ...
                    GLPrimitives.Draw2DCircleBorderLines(rotationSphereScreenSpaceBoundaryPoints, sphereCenterInScreenSpace, circleColor, _cameraLookRotationCircleRadiusScale, _lineRenderingMaterial, _camera);
                }
            }
        }
        #endregion
        ...
        private void DrawRotationCircle(GizmoAxis gizmoAxis)
        {
            ...
            // Draw the circle
            GLPrimitives.Draw3DLines(worldSpaceCirclePoints, circlePointColors, true, _lineRenderingMaterial, true, circlePointColors[0]);
        }
        ...
        /// <summary>
        /// Creates all the materials which are necessary to render the rotation sphere object.
        /// </summary>
        private void CreateRotationSphereMaterials()
        {
            // Create the shaders which must be associated with the materials. 2 shaders are needed
            // because the user can choose whether or not the rotation sphere must be lit.
            Shader litSphereShader = Shader.Find("Gizmo Lit Mesh");
            Shader unlitSphereShader = Shader.Find("Gizmo Unlit Mesh");

            // Create the lit sphere material
            _litRotationSphereMaterial = new Material(litSphereShader);
            _litRotationSphereMaterial.SetColor("_Color", _rotationSphereColor);

            // Create the unlit sphere material
            _unlitRotationSphereMaterial = new Material(unlitSphereShader);
            _unlitRotationSphereMaterial.SetColor("_Color", _rotationSphereColor);
        }

        /// <summary>
        /// Creates the material which is used to render the rotation guide disc.
        /// </summary>
        private void CreateRotationGuideDiscMaterial()
        {
            // Create the shader which must be used to render the rotation disc
            Shader shader = Shader.Find("Gizmo Unlit Mesh");

            // Create the rotation disc material
            _rotationGuideDiscMaterial = new Material(shader);
            _rotationGuideDiscMaterial.SetColor("_Color", _rotationGuideDiscColor);
        }

        /// <summary>
        /// Assigns the rotation sphere color to the rotation sphere materials.
        /// </summary>
        private void AssignRotationSphereColorToSphereMaterials()
        {
            if (_litRotationSphereMaterial != null) _litRotationSphereMaterial.SetColor("_Color", _rotationSphereColor);
            if (_unlitRotationSphereMaterial != null) _unlitRotationSphereMaterial.SetColor("_Color", _rotationSphereColor);
        }

        /// <summary>
        /// Returns the material which is used to render the rotation sphere object. This material is
        /// different based on whether or not the rotation sphere must be lit.
        /// </summary>
        private Material GetRotationSphereMaterial()
        {
            return _isRotationSphereLit ? _litRotationSphereMaterial : _unlitRotationSphereMaterial;
        }

        /// <summary>
        /// Creates the sphere mesh which is used to render the rotation sphere.
        /// </summary>
        private void CreateRotationSphereMesh()
        {
            const int numberOfVerticalSlices = 50;
            const int numberOfStacks = 50;

            // Destroy the old mesh if it already exists and then create a new one.
            // Note: It is important to destroy the old mesh because this method might be called in
            //       response to a user's request to change the sphere's properties (e.g. radius) and
            //       in that case the old mesh has to be destroyed and a new one created.
            if (_rotationSphereMesh != null) DestroyGizmoMesh(_rotationSphereMesh);
            _rotationSphereMesh = ProceduralMeshGenerator.CreateSphereMesh(_rotationSphereRadius, numberOfVerticalSlices, numberOfStacks);
        }
        ...
        {
            // In order to avoid having to create a game object for the sphere, we will just 
            // use the 'Graphics' API to perform the rendering.
            // Note: Please see the Unity Scripting API documentation to understand how this function
            //       can be used: http://docs.unity3d.com/ScriptReference/Graphics.DrawMesh.html
            Graphics.DrawMesh(_rotationSphereMesh, worldTransform, GetRotationSphereMaterial(), gameObject.layer);
        }
        ...
        {
            // This is just the gimo's world transform
            return _gizmoTransform.localToWorldMatrix;
        }
