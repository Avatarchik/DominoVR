...
namespace RTEditor
{
    ...
    public class EditorGizmoSystem : MonoSingletonBase<EditorGizmoSystem>, IMessageListener
    {
        ...
        /// <summary>
        /// This is a directional light which lights only the gizmo objects. The system will
        /// automatically create this light at startup.
        /// </summary>
        [SerializeField]
        private GameObject _gizmoDirectionalLight;

        /// <summary>
        /// This is the layer in which all gizmo objects will reside. It is used to make
        /// sure the directional light will light only the gizmo objects. We will use the
        /// layer number 0 by default.
        /// </summary>
        [SerializeField]
        private int _gizmoLayer = 0;
        ...
        /// <summary>
        /// Gets/sets the number of the layer to which the gizmo objects belong. The proprty
        /// will clamp the layer number accordingly.
        /// </summary>
        public int GizmoLayer
        {
            get { return _gizmoLayer; }
            set
            {
                _gizmoLayer = Mathf.Max(0, Mathf.Min(31, value));

                // Make sure the gizmo objects have their layers adjusted and also update the
                // directional light's culling mask such that it only affects the gizmo objects.
                AssignGizmosToGizmoLayer();
                AdjustDirectionalLightCullingMask();
            }
        }

        /// <summary>
        /// Gets/sets the name of the layer to which the gizmo objects belong. The property
        /// performs no validation on the specified layer name.
        /// </summary>
        public string GizmoLayerName
        {
            get { return LayerMask.LayerToName(_gizmoLayer); }
            set
            {
                // Adjust the gizmo layer number based on the specified name
                _gizmoLayer = LayerMask.NameToLayer(value);
        ...
        private void Start()
        {
            ...
            AssignGizmosToGizmoLayer();                     // Make sure the gizmos are assigned to the user specified layer
            CreateGizmoDirectionalLight();                  // Create the gizmo directional light
            AdjustDirectionalLightCullingMask();            // Make sure the directional light's culling mask is adjusted so that the light only affects the gizmo objects
        ...
        /// <summary>
        /// Assigns the gizmo objects to the user specified gizmo layer.
        /// </summary>
        private void AssignGizmosToGizmoLayer()
        {
            if (_translationGizmo != null) _translationGizmo.gameObject.SetLayerForEntireHierarchy(_gizmoLayer);
            if (_rotationGizmo != null) _rotationGizmo.gameObject.SetLayerForEntireHierarchy(_gizmoLayer);
            if (_scaleGizmo != null) _scaleGizmo.gameObject.SetLayerForEntireHierarchy(_gizmoLayer);
        }

        /// <summary>
        /// Creates the gizmo directional light if not already created.
        /// </summary>
        private void CreateGizmoDirectionalLight()
        {
            if(_gizmoDirectionalLight == null)
            {
                // Create the light object
                _gizmoDirectionalLight = new GameObject();
                _gizmoDirectionalLight.name = "Gizmo Directional Light";

                // Attach a directional light component to the light object
                Light lightComponent = _gizmoDirectionalLight.AddComponent<Light>();
                lightComponent.type = LightType.Directional;
                lightComponent.shadows = LightShadows.None;
                lightComponent.intensity = 1.5f;
                lightComponent.cullingMask = 1 << _gizmoLayer;
             
                // The light will always have the same orientation of as that of the editor camera.
                // It would make things tidy to make the light a child of the camera and adjust its
                // position so that it always corresponds to that of the camera object.
                Transform lightTransform = _gizmoDirectionalLight.transform;
                lightTransform.parent = EditorCamera.Instance.transform;
                lightTransform.position = EditorCamera.Instance.transform.position;
            }
        }

        /// <summary>
        /// Adjusts the culling mask of the directional light so that it only lights the gizmo objects.
        /// </summary>
        private void AdjustDirectionalLightCullingMask()
        {
            if (_gizmoDirectionalLight != null) _gizmoDirectionalLight.GetComponent<Light>().cullingMask = 1 << _gizmoLayer;
        }
        ...
        private void Update()
        {
            // Make sure we always have a gizmo directional light available
            if (_gizmoDirectionalLight == null)
            {
                CreateGizmoDirectionalLight();
                AdjustDirectionalLightCullingMask();
            }

            // Perform any necessary updates on teh gizmo directional light
        ...
        private void EstablishActiveGizmoPosition()
        {
            ...
            EditorObjectSelection objectSelection = EditorObjectSelection.Instance;
            if (_activeGizmo.gameObject.activeSelf && !_activeGizmo.IsTransformingObjects())
            {
        ...
        /// <summary>
        /// Applies any necessary updates on the directional light which follows the camera. This
        /// method only has effect if a directional light has been specified via the Inspector GUI.
        /// </summary>
        private void UpdateDirectionalLight()
        {
            // If a directional light was specified, make sure it has the same orientation as the camera
            // and also ensure that no colliders are attached to it so that it can not be selected in the scene.
            if (_gizmoDirectionalLight != null)
            {
                _gizmoDirectionalLight.RemoveAllColliders();
                _gizmoDirectionalLight.transform.rotation = EditorCamera.Instance.transform.rotation;
            }
        }
        #endregion
