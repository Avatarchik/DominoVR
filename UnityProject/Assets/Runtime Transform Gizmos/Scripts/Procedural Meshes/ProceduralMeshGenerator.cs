    ...
    public static class ProceduralMeshGenerator
    {
        ...
        public static Mesh CreateConeMesh(float bottomRadius, float height, int numberOfVerticalSlices, int numberOfHorizontalSlices, int numberOfBottomCapRings)
        {
            ...
            for (int horizontalRingIndex = 0; horizontalRingIndex < numberOfHorizontalVertexRings; ++horizontalRingIndex)
            {
                ...
                {
                    // Generate the normal. 
                    // Note: The Y coordinate of the normal is always 0 because it is just an orientation vector.
                    coneVertexNormals[currentVertexIndex] = new Vector3(Mathf.Cos(Mathf.Deg2Rad * angleStep * vertexIndex), 0.0f,
                                                                        Mathf.Sin(Mathf.Deg2Rad * angleStep * vertexIndex));

                    // Use the normal to calculate the vertex position.
                    // Note: The vertex position will have to have its Y position set accordingly.
                    coneVertexPositions[currentVertexIndex] = new Vector3(coneVertexNormals[currentVertexIndex].x * currentRadius,
                                                                           currentHeight,
                                                                           coneVertexNormals[currentVertexIndex].z * currentRadius);
